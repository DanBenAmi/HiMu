...
class YOLOExpert(BaseExpert):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "ultralytics is required for YOLOExpert. "
                "Install with: pip install 'himu[yolo]'"
            )
        ...
        yolo_weights_dir = self.weights_dir / "yolo"
        yolo_weights_dir.mkdir(parents=True, exist_ok=True)

        model_filename = f"{model_name}.pt"
        local_weights_path = yolo_weights_dir / model_filename

        if local_weights_path.exists():
            log.info(f"Loading YOLO weights from: {local_weights_path}")
            model_to_load = str(local_weights_path)
        else:
            log.info(f"YOLO weights not found in {yolo_weights_dir}, will download and save")
        ...
        for source in possible_sources:
            if source.exists():
                try:
                    ...
                    log.info(f"Copied YOLO weights to: {target_path}")
                    return
                except Exception as e:
                    log.warning(f"Failed to copy YOLO weights: {e}")

        log.warning(f"Could not find downloaded YOLO weights to copy to {target_path}")
    ...
    def compute_batch_scores(
        ...
        """
        Batch YOLO detection across multiple queries with frame batching.
